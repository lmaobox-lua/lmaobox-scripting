...
-- purpose: ChatPrintf color codes
...
-- example case: formatc('%s3 %s2 over the %s1 dog %s2 %s3', 'lazy', 'jumps', 'a fox')
local format_cstr = function( fmt, ... )
    local resource, order = { ... }, {}
    -- lua brainfuck: the parentheses around string.gsub force the expression into a single value (the first value or nil)
    -- http://www.lua.org/manual/5.4/manual.html#3.4
    fmt = (fmt:gsub( '%%s(%d+)', '%%%1' ))
    fmt = fmt:gsub( '%%(%d+)', function( i )
        table.insert( order, resource[tonumber( i )] or '' )
        return '%s'
    end )
    fmt = string.format( fmt, table.unpack( order ) )
    return fmt
end

local localize_find = function( key, if_fail )
    local s = client.Localize( key )
    s = (#s > 0 and s ~= nil) and s or if_fail or key
    return s
end

local localize_find_and_format = function( key, ... )
    local text = localize_find( key )
    return format_cstr( text, ... )
end
...
-- @param id: name: lua callbacks
local listener = function( id )
    local e = {}
    e.add = function( callback )
        local unique = (id .. '_callback_tmp_' .. engine.RandomInt( 0, 0x7FFF ))
        callbacks.Unregister( id, unique )
        callbacks.Register( id, unique, function( userdata ) callback( userdata ) end )
        return unique
    end
    return e
end
...
--[[
    globals.CurTime(): Returns the current time <number: second.millsecond (millsecond: 10 digit)>
    globals.RealTime(): Returns the time since start of the game <number: second.millsecond (millsecond: 10 digit)>
    os.time(): returns os current time in epoch seconds <number>
]]

local setTimeout = function( interval, callback )
    local time = globals.RealTime() + interval
    local id, unique = 'Draw', (id .. '_callback_tmp_' .. engine.RandomInt( 0, 0x7FFF ))
...
local setInterval = function( interval, callback )
    local time = globals.RealTime() + interval
    local id, unique = 'Draw', (id .. '_callback_tmp_' .. engine.RandomInt( 0, 0x7FFF ))
    callbacks.Register( id, unique, function()
...
callbacks.Register( 'Unload', function()
    ...
    local r, g, b, a = table.unpack( to_rgba( '#39b83dff' ) )
    printc( r, g, b, a, string.format( '[%s | %s] previous instance:\n%s unloaded', os.date( '%c' ), globals.RealTime(), GetScriptName() ) )
end )
...
um.add( function( msg )
    ...
    ---
    local s = localize_find_and_format( disp_str, details_str )
...
um.add( function( msg )
    ...
    ---
    local s = localize_find_and_format( vote_failed_localize[reason] )
