...
callbacks.Register( 'PostPropUpdate', function()
    local tonemap = entities.FindByClass( 'CEnvTonemapController' )
    for i = 1, 1000 do
        local ent = entities.GetByIndex( i )
        if ent and ent:GetClass() == 'CEnvTonemapController' then
            tonemap = ent
            break
        end
    end

    tonemap:SetPropBool( true, 'm_bUseCustomAutoExposureMin' )
    tonemap:SetPropBool( true, 'm_bUseCustomAutoExposureMax' )
    tonemap:SetPropBool( true, 'm_bUseCustomBloomScale' )
    tonemap:SetPropFloat( 100, 'm_flCustomAutoExposureMin' )
    tonemap:SetPropFloat( 100, 'm_flCustomAutoExposureMax' )
    tonemap:SetPropFloat( 100, 'm_flCustomBloomScale' )
end )]]


-- DT_FogController, DT_EnvScreenEffect, DT_CaptureFlag
...
callbacks.Register( 'PostPropUpdate', function()
    for i = 1, 2048 do
        ...
        if ent:GetClass() == 'CFogController' then
            ent:SetPropInt( 1, 'm_fog.enable' )
            ent:SetPropFloat( 255, 'm_fog.colorPrimary' )
            ent:SetPropFloat( 1000, 'm_fog.start' )
            ent:SetPropFloat( 1500, 'm_fog.end' )
        end
    end
end )

if mat:GetName():find( 'models/props' ) then
    mat:SetShaderParam( '$color', Color( 157, 71, 71 ) )
    mat:SetShaderParam( '$alpha', 0 )
