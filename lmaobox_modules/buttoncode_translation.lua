local button_code_name, steam_controller_buttons, steam_controller_buttons_axis, steam_controller_buttons_virtual
...
do
    local index = #button_code_name + 1
    local order = {
        steam_controller_buttons,
        steam_controller_buttons_axis,
        steam_controller_buttons_virtual
    }
    for _, table in ipairs(order) do
        local copy = 1
        while copy ~= 7 do
            for keycode, value in ipairs(table) do
                button_code_name[index] = value
                index = index + 1
            end
            copy = copy + 1
        end
...
assert(button_code_name[255] == 'SC_RPAD_DOWN', string.format("assertion failed: expected 'SC_RPAD_DOWN', got '%s' instead", button_code_name[255]))
return button_code_name
